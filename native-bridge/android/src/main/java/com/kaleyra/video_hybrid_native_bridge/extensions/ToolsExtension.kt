...
import com.kaleyra.video_common_ui.ChatUI.Action.ShowParticipants
...
internal fun Tools.toChatActions(): Set<ChatUI.Action> {
    ...
    when (chat.audioCallOption?.type) {
        Audio -> {
            chatActions.add(CreateCall(preferredType = Call.PreferredType.audioOnly()))
        }
        AudioUpgradable -> {
            chatActions.add(CreateCall(preferredType = Call.PreferredType.audioUpgradable()))
        }
        else -> Unit
    }
    if (chat.videoCallOption != null) {
        chatActions.add(CreateCall(preferredType = Call.PreferredType.audioVideo()))
    }
    return chatActions
}
